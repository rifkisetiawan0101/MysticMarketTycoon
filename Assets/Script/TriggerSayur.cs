...
public class TriggerSayur : MonoBehaviour {
    ...
    private void Start() {
        ...
        openInfoButton.GetComponent<Button>().onClick.AddListener(() => {
            ...
            infoSayurCanvas.SetActive(true);
    ...
    private void TambahPenghasilanMerchant(Collider2D collision) {
        ...
        if (merchantData.penghasilanMerchant >= (merchantData.hargaDagangan * 3)) {
            collectButton.SetActive(true);
            TutorialManager.Instance.isKoinActive = true;
        }
    }
    ...
    private void OnCollectButtonClick() {
        ...
        merchantData.penghasilanMerchant = 0;
        Debug.Log("Penghasilan Sayur Saat Ini = " + merchantData.penghasilanMerchant);
        collectButton.SetActive(false);
        StartCoroutine(FeedbackManager.instance.PlayCollectKoin());

        TutorialManager.Instance.isKoinActive = false;
    }
