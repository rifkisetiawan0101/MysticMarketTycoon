...
public class CollectAkik : MonoBehaviour, IPointerDownHandler, IPointerUpHandler {
    [SerializeField] private GameObject akikPrefab;
    ...
    private void Update() {
        if (isHolding && ColTriggerAkik.isPlayerInRange & !hasCollected) {
            ...
            if (holdTimer >= holdTime) {
                hasCollected = true;
                tapHold.SetActive(false);
                PersistentManager.Instance.UpdateCollectable(1, "Akik");
                TutorialManager.Instance.isItemCollected = true;
                StartCoroutine(FeedbackManager.instance.PlayAkikCollect(akikPrefab));
            }
        }
    }
    ...
    public void OnPointerDown(PointerEventData eventData) {
        if (ColTriggerAkik.isPlayerInRange) {
