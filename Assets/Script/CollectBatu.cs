...
public class CollectBatu : MonoBehaviour, IPointerDownHandler, IPointerUpHandler {
    ...
    private void Update() {
        if (isHolding && ColTriggerBatu.isPlayerInRange && !hasCollected) {
            ...
            if (holdTimer >= holdTime) {
                hasCollected = true;
                tapHold.SetActive(false);
                PersistentManager.Instance.UpdateCollectable(1, "Batu");
                TutorialManager.Instance.isItemCollected = true;
                StartCoroutine(FeedbackManager.instance.PlayBatuCollect(batuPrefab));
            }
        }
    }
    ...
    public void OnPointerDown(PointerEventData eventData) {
        if (ColTriggerBatu.isPlayerInRange) {
